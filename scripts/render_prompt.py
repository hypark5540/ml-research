...
from ml_research.models import DigestArchive, DigestContext, WeeklyDigest
...
def main() -> int:
    ...
    archive_payload = read_json(args.archive)
    start_date = args.week_of - timedelta(days=7)
    schema = WeeklyDigest.model_json_schema()
    if isinstance(archive_payload, dict) and "digests" in archive_payload:
        archive = DigestArchive.model_validate(archive_payload)
        existing_ids = sorted(
            paper["arxivId"]
            for digest in archive.model_dump(mode="json")["digests"]
            for paper in digest["papers"]
        )
    else:
        context = DigestContext.model_validate(archive_payload)
        existing_ids = sorted(context.arxivIds)

    prompt = f"""\
Create one public bilingual weekly ML research digest for HYOYOUL BLOG.

You have only read-only paper/search tools and one trusted submission tool. You
cannot run a shell or read environment variables. When the digest is complete,
call `submit_weekly_digest` with the entire object. Do not print JSON as the final
answer: success requires that the trusted submission tool accepts it.
...
7. Submit one object matching the schema below. Set:
   id = research-{args.week_of.isoformat()}
   slug = weekly-ml-research-digest-{args.week_of.isoformat()}
   weekOf = {args.week_of.isoformat()}
   version = 1
8. Call `submit_weekly_digest` and fix every validation error before finishing.
...
- The JSON Schema below and the trusted submission tool fully define the output.
...
- Success requires the trusted submission tool to accept the digest. A text-only
  research summary is not a completed task.
